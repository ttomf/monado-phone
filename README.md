...
This is a driver for the Monado XR runtime that targets **Android phones**. It comes with an app that allows you to use Monado on your phone. Currently, it has **UDP streaming** and **6DOF tracking** support.
...
In one terminal, run the **Monado service**:
```sh
build/src/xrt/targets/service/monado-service
```

In another terminal, run your OpenXR application with **environment variable** set (you will need to provide full path to the OpenXR runtime configuration):
```sh
...
```
XR_RUNTIME_JSON=build/openxr_monado-dev.json PRESSURE_VESSEL_FILESYSTEMS_RW=/run/user/$UID/monado_comp_ipc %command%
```
...
sequenceDiagram
  ...
  Note over P: Phone is repeatedly<br/>sending UDP multicast
  G->>M: Game requests OpenXR runtime
  M->>D: Monado probes all drivers
  P->>D: Phone sends UDP multicast
  Note over D: Driver receives multicast,<br/>saves phone IP and responds
  ...
  Note over P: Phone initializes ARCore<br/>and stream receiver
  Note over D: Driver initializes Vulkan compositor target<br/>with gstreamer and pose receiver
  ...
  par 
    loop Rendering
      ...
      Note over P: Phone renders the<br/>received frame on Surface
    end
    loop Pose Data
...
**Extent of use**: AI was used to generate low-level boilerplate code and setup modules, primarily for Vulkan management & rendering setup. I rewrote the AI-generated code line by line and optimized it. This README was not generated by AI.
